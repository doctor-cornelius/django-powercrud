...
def object_list(context, objects, view):
    ...
    if list_column_state and getattr(list_column_state, "enabled", False):
        active_columns = set(getattr(list_column_state, "active_columns", []) or [])
        fields = [field_name for field_name in fields if field_name in active_columns]
        properties = [
            prop_name for prop_name in properties if prop_name in active_columns
        ]
    ...
    for row_index, obj in enumerate(objects):
        ...
        if inline_enabled and inline_row_endpoint:
            inline_row_url = view.safe_reverse(
                inline_row_endpoint, kwargs={"pk": obj.pk}
            )
...
def get_proper_elided_page_range(paginator, number, on_each_side=1, on_ends=1):
    ...
    page_range = paginator.get_elided_page_range(
        number=number, on_each_side=1, on_ends=1
    )
    return page_range
