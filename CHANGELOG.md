...
- **Fix (list)**: do not override cell tooltips for uneditable rows (#252)
- **Fix (tests)**: resolve image PCRE scan and tooltip race

## 0.9.12 (2026-09-05)

- **Chore(docs)**: replace `mkdocs` and `mkdocs-material` with `zensical` for docs generation
...
- **Style (list)**: narrow padding on standard row action icons

## 0.9.8 (2026-08-08)

- **Style(actions)**: increase contrast of row action icons
- **Chore(sample)**: provide management command `runbootstrap` for sample app to run up bootstrap5 server
...
- **Feature (Django)**: add official Django 6.0 support while retaining Django 5.2 compatibility
- **Feature (templates)**: add a template partial compatibility shim for Django 5.2/6.0 projects
...
- **Fix (table)**: import typing.Any for link field hints to satify python <3.14 tests
