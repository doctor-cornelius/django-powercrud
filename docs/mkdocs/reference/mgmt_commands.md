...
The starter has no framework-name registration. Its generated declaration pins the current numeric template-pack contract version (`contract_version=5`); it does not import a moving current-version constant. Edit the copied templates and the optional adapter hooks, then test and publish it as a normal Python distribution. A later PowerCRUD contract bump is an explicit pack upgrade obligation. See [Authoring and publishing a template pack](../template_packs/authoring-and-publishing.md) for the complete workflow.
...
The component receives `object` and `row_actions`. PowerCRUD has already resolved URLs, permissions, visibility, disabled and lock state, HTMX behavior, modal behavior, lazy-state indices, and framework classes. Its version-3 context includes `standard_actions`, `extra_actions`, mode-specific `dropdown_actions`, canonical `responsive_dropdown_actions`, compatibility `standard_dropdown_actions` and `extra_dropdown_actions`, `show_dropdown`, `show_extra_dropdown`, `show_all_dropdown`, `show_responsive_dropdown`, `dropdown_scope`, and the lazy-state URL. The override should render only that supplied presentation data; do not repeat authorization, state-hook, order, or URL decisions in the template.

You can change the arrangement, classes, labels, and icons without copying PowerCRUD JavaScript or the list template. Preserve the accessible extras-only and all-actions kebabs, canonical labelled responsive order, standard icon gutter, semantic action-kind/destructive markers, and conditional Delete separation. If the override retains responsive behavior, expose the desktop and mobile groups through the supplied semantic markers and never show both simultaneously. Keep menu rows neutral rather than reusing configured extra-button fills. Preserve `data-inline-action` plus the floating dropdown, lazy-state, disabled-tooltip, HTMX, modal, and modal-close-refresh attributes used by the behavior you retain. The legacy `action_links()` function and `row.actions`/`row.has_actions` list-template values remain available during the 0.x compatibility period.
...
You can change header layout, classes, labels, sort icons, and help icons without copying PowerCRUD JavaScript or the whole list template. Keep `headers`, `current_sort`, `filter_params`, `use_htmx`, `request`, `has_row_actions`, `extra_actions_mode`, `enable_selection_controls`, `row_actions_column_position`, `row_actions_column_sticky`, and the existing selection-state context available. Preserve the sorting URL and HTMX attributes, semantic help trigger, conditional Actions heading, responsive heading hook, action-column markers, and select-all hooks for the behavior you retain. In `all_dropdown`, omit the visible heading but retain screen-reader-only **Actions** text; the first-party packs also compact it below 640px for every mode. Selection remains the outermost, always-sticky logical-start system column, and sticky start actions follow it. The table shell and rows remain separately overridable, while the select-all column delegates to the focused bulk-selection controls component.
