...
# Django 6.1 core suite
uv pip install -c requirements/constraints-django61.txt ".[tests-core]"
pytest -m "not playwright"
...
# Django 6.1 Playwright smoke subset
uv pip install -c requirements/constraints-django61.txt ".[tests-core]" ".[tests-ui]"
npm ci
npm run build
pytest -m playwright_smoke

# Django 6.1 full Playwright suite
uv pip install -c requirements/constraints-django61.txt ".[tests-core]" ".[tests-ui]"
npm ci
npm run build
pytest -m playwright
...
- Use `requirements/constraints-django52.txt` or `requirements/constraints-django61.txt` when you need to reproduce the exact support-matrix environment rather than the default local workflow.
