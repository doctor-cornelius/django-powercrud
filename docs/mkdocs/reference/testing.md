...
- Broader browser regression coverage: run the full Playwright suite separately from the smoke gate, so frontend regressions stay visible without making every nuanced browser interaction a merge blocker.
...
- GitHub Actions blocks on the curated `playwright_smoke` subset. The full Playwright suite is still intended for local runs, release preparation, and separate non-blocking CI visibility on `main`.
