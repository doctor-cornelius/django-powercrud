...
- **AsyncTaskRecordCRUDView**: The temporal list-format sample. It uses `default_datetime_value_format = "date"`, shows `updated_at` as time-only, `completed_at` as date-and-time, and leaves `failed_at` on the date-only view default.
