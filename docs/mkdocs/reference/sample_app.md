...
The **Authors** list combines `extra_actions_mode = "all_dropdown"` with a logical-start sticky action column. Its compact vertical-ellipsis trigger follows the selection checkbox, the visible header label is omitted, and the menu shows permitted View/Edit/Delete controls as a coloured icon row above the labelled Home and View Again extras. Inline Save/Cancel temporarily replace that trigger and the compact menu returns after the HTMX row replacement.
...
- **AuthorCRUDView**: Properties, filtering, template debugging, companion nullable scalar filtering on `birth_date`, the sample app's red inline-edit highlight accent demo, the compact all-actions menu with coloured standard icons and labelled extras, and the combined logical-start-plus-sticky action column with selection and inline editing enabled
...
- **PowerFieldBookCRUDView**: The same Book fields, default columns, toolbar actions, and row-action declarations expressed through PowerField, PowerButton, and PowerAction, with the compact all-actions menu on the logical-end sticky column for comparison with Authors and Books
...
4. **Test features:**
   ...
   - Experiment with the restricted Books page sizes
