...
| `templates_path` (`str`) | `str` | `powercrud/{POWERCRUD_CSS_FRAMEWORK}` | Uses bundled templates for the selected compatible path | Base directory for explicit whole-template overrides. Prefer focused component overrides for normal customization. | [Template Packs](../template_packs/index.md) |
...
| `use_crispy` (`bool`) | `None`, `True`, `False` | `None` | Auto-detects: `True` when `crispy_forms` is installed | Toggle Crispy Forms rendering for generated forms. The application chooses its Crispy pack; see the tested first-party combinations. | [Template Packs](../template_packs/index.md#forms-and-crispy) |
...
| `POWERCRUD_TEMPLATE_PACK` (`str`) | absent, built-in alias `daisyui`, or template-pack declaration path | absent | Selects the supported DaisyUI default | Select the complete template pack at process startup. Use `powercrud.contrib.bootstrap5:template_pack` for Bootstrap 5. | [Template Packs](../template_packs/index.md) |
| `POWERCRUD_CSS_FRAMEWORK` (`str`) | `str`; bundled default: `'daisyUI'` | `'daisyUI'` | Retains the compatible DaisyUI styling path | Legacy CSS-framework compatibility setting. It does not select an optional template pack such as Bootstrap. | [Template Packs](../template_packs/index.md) |
