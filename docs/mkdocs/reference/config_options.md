...
| `extra_button_selection_controls_disabled` (`bool`) | `True`, `False` | `False` | Selection-aware extra buttons can render row selection controls | When `True`, `extra_buttons` with `uses_selection=True` do not cause row selection controls to render. `bulk_fields` and `bulk_delete` still render selection controls because built-in bulk operations require them. | [Setup & Core CRUD basics](../guides/setup_core_crud.md#extra-buttons) |
...
| `list_cell_tooltip_fields` (`dict[str, str]`) | `None`, `dict[str, str]`, or deprecated `list[str]` | `None` | No semantic list-cell tooltips are rendered | Map rendered list fields/properties to row-specific tooltip hook methods. PowerCRUD only evaluates configured names that are actually visible in the current list and silently ignores configured names that are not rendered. Hook-backed semantic cell tooltip text may include newline characters for multiline display. The legacy list form is deprecated. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
...
- Set `extra_button_selection_controls_disabled = True` only when a view needs a selection-aware endpoint but should not expose row selection controls from extra buttons. Built-in bulk edit/delete still render selection controls because they require them.
