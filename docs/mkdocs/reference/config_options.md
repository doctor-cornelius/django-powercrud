...
| `default_list_fields` (`list[str]`) | `None` or non-empty `list[str]` | `None` | All allowed list columns render and no column chooser is shown | Limit the default visible list-column subset while keeping the remaining allowed `fields` and `properties` available through the session-backed **Cols** control. Model fields, queryset annotation fields, and properties are supported. | [List Options](../guides/advanced/list_options.md) |
