...
| `extra_buttons` (`list[dict \| PowerButton]`) | `list[button spec]` | `[]` | No extra header buttons are shown | Add top-of-page buttons (e.g., custom actions, links). Buttons with `uses_selection=True` can render row selection controls even when built-in bulk edit/delete is not configured. Selection-consuming buttons may set `clear_selection_on_success=True`. Modal buttons may set per-trigger `modal_box_classes`, `refresh_list_on_modal_close`, and permission affordance fields. | [Complete Example](complete_example.md) |
...
| `paginate_by` (`int`) | `None` or `int` | `25` | Lists render 25 rows per page | Page size for list views; users can override via `?page_size=`. Set `None` on a view to render all rows by default. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
...
- Set `clear_selection_on_success = True` only for HTMX buttons that consume the current selection. PowerCRUD clears the persisted selection after that button request succeeds; failed requests and ordinary summary/modal-preview buttons leave the selection intact.
...
??? info "Parameter Guide"
    ...
    | `clear_selection_on_success` | `bool` | Clears the persisted selection after a successful HTMX request from a selection-aware button. Defaults to `False`; ignored unless `uses_selection=True`. |
