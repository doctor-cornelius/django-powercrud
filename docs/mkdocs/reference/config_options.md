...
| `column_width_modes` (`dict[str, str]`) | `None` or `dict[str, 'compact' \| 'auto' \| 'bounded']` | `None` | The view-wide `column_width_policy` decides each column | Override the resolved list width mode for individual fields, annotations, or configured properties. | [Styling & Tailwind](../guides/styling_tailwind.md) |
| `column_width_policy` (`str`) | `'bounded'`, `'semantic'` | `'bounded'` | All columns retain the legacy bounded width | Opt into type-aware list widths: compact for automatic primary keys and booleans, auto for temporal and numeric fields, and bounded for text, relations, annotations, and properties. | [Styling & Tailwind](../guides/styling_tailwind.md) |
