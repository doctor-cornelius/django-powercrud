...
| `filter_favourites_enabled` (`bool`) | `True`, `False` | `False` | No saved-favourites toolbar is rendered | Enable the optional saved filter favourites UI for this list view when the `powercrud.contrib.favourites` app is installed. | [Saved Filter Favourites](../guides/advanced/filter_favourites.md) |
