...
| `default_datetime_value_format` (`str`) | `'date'`, `'time'`, `'datetime'` | `'datetime'` | DateTimeField columns show date and time | View-wide fallback for unconfigured `DateTimeField` and typed datetime-annotation list columns. Named `column_value_formats` entries take precedence. Formatting uses Django's active timezone plus `DATE_FORMAT`, `TIME_FORMAT`, or `DATETIME_FORMAT`. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
...
| `link_fields` (`dict[str, str \| dict]`) | `None` or mapping of rendered field/property name to a `view_name` string or a dict with exactly one of `view_name` / `url`, plus optional `pk_attr` / `open_in` / `modal_box_classes` | `None` | List cells render as plain text/value output | Make selected rendered list cells clickable. String shorthand uses the named view plus a default pk source (`<field>_id` for relation fields, row `pk` otherwise). Dict form may reverse a Django `view_name` or use a static `url`. Omitted `open_in` values use `list_cell_link_default_open_in`, whose own omitted default is `"new"`; explicit values may be `"current"`, `"new"`, or `"modal"`. Modal links may set `modal_box_classes` for per-cell modal sizing. Inline-editable cells are never linked. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
...
| `templates_path` (`str`) | `str` | `powercrud/{POWERCRUD_CSS_FRAMEWORK}` | Uses bundled templates for the configured framework | Base directory for all template overrides. | [Customisation tips](../guides/advanced/customisation_tips.md) |
...
| `view_help` (`dict`) | `None` or `{"summary": str, "details": str, "default_open": bool, "color": str, "min_width": str}` | `None` | No collapsed screen help is shown | Render optional collapsed screen-level help below `view_instructions` and above the list toolbar. Content is escaped plain text; blank lines in `details` create paragraphs. Optional `color` accepts `base`, daisyUI semantic colours, or hex. Optional `min_width` overrides the view default. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
| `view_help_default_color` (`str`) | `base`, daisyUI semantic colour, or hex colour | `base` | Collapsed screen help uses the quiet base colour | Default colour theme for `view_help` when the help dict does not provide `color`. Semantic and hex colours are rendered as subtle tints, with the summary bar slightly stronger than the content area. | [Setup & Core CRUD basics](../guides/setup_core_crud.md) |
