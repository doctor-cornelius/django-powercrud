...
- `extra_button_selection_controls_disabled = True` suppresses selector controls caused by selection-aware `extra_buttons` only. It does not suppress selector controls required by `bulk_fields` or `bulk_delete`.
