...
- `clear_selection_on_success = True` clears the persisted selection after a successful HTMX request from a selection-aware header button. Use it for selection-consuming actions, not summary or preview modals that should preserve the user's selection.
