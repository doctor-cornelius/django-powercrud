...
PowerCRUD still evaluates permission checks during list rendering. It defers only the hooks that opt into lazy mode, then resolves them when the row's **More** or **Actions** menu opens.
...
Lazy row-action state is supported by both `extra_actions_mode = "dropdown"` and `"all_dropdown"`. The extras-only **More** menu and compact all-actions **Actions** menu use the same stable configured-action indices and state endpoint. Visible button-mode row actions stay eager.
