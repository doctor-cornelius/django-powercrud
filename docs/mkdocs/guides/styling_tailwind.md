...
The list toolbar defaults to `list_toolbar_width_policy = "container"` and `list_toolbar_alignment = "adjacent"`. This keeps view controls beside the action group without pushing them to the viewport edge or squeezing them to a compact table's width. Use `list_toolbar_width_policy = "table"` and/or `list_toolbar_alignment = "split"` when a view needs the legacy table-aligned arrangement. Both DaisyUI and Bootstrap 5 implement these settings with logical spacing and natural narrow-screen wrapping.
