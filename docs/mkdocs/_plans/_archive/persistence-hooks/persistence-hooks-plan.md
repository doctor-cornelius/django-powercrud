...
1. [x] Document migration and adoption guidance after the sync release lands.
    - [x] Add migration guidance for downstream projects currently overriding internal methods once the final sync API is confirmed in released docs.
    - [x] Record any release-note level caveats or upgrade notes needed for maintainers.
2. [x] Document the async backend story after it lands.
    - [x] Add docs for async bulk backend resolution.
    - [x] Confirm that standalone `AsyncManager` reuse is not being productized as part of this plan phase.
3. [x] Document limitations and non-goals explicitly.
