...
A focused `row-actions` override receives the already resolved `standard_dropdown_actions`, `extra_dropdown_actions`, compatibility `dropdown_actions`, dropdown-scope flags, and lazy-state URL. In `all_dropdown` mode, render permitted standard actions as a centred icon-only group with their supplied classes and semantic tooltips, then render labelled configured extras with their supplied menu classes. Omit an empty standard group and preserve the trigger, floating-template, action-kind, lazy-state, disabled-tooltip, HTMX, modal, and refresh hooks. An override that changes only these controls does not own column placement.
