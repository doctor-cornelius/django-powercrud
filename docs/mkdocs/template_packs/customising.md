...
Contract-version-5 list overrides receive `extra_buttons_dropdown_label`, `extra_actions_mode`, `row_actions_column_position`, `row_actions_column_sticky`, `list_toolbar_width_policy`, and `list_toolbar_alignment`. If an override owns system-column structure, it must apply the configured order consistently to `table-header`, `table-row` / `inline-row-display`, and `inline-row-form`; selection remains always sticky and outermost at logical start. Preserve `data-powercrud-selection-column`, `data-powercrud-row-actions-column`, `data-powercrud-row-actions-position`, and the conditional `data-powercrud-row-actions-sticky` marker so pack CSS and browser-focused tests can identify and offset the semantic columns without depending on framework classes. If an override owns the list toolbar, preserve its width-policy and alignment markers, use the PowerCRUD list container rather than the browser viewport for `container`, and keep adjacent controls naturally wrappable. In `all_dropdown` mode the visible heading is omitted but retains screen-reader-only **Actions** text.

A focused `row-actions` override receives mode-specific `dropdown_actions`, canonical `responsive_dropdown_actions`, compatibility `standard_dropdown_actions` and `extra_dropdown_actions`, desktop and responsive visibility flags, dropdown-scope flags, and the lazy-state URL. In `all_dropdown` mode, render `dropdown_actions` as one labelled list in its supplied order. At the pack's narrow breakpoint, hide the desktop group and render `responsive_dropdown_actions` in all-actions scope for every mode. Standard entries include menu icons and configured extras intentionally leave that icon gutter empty. Do not reapply configured extra-button fills inside either menu. Preserve the accessible triggers, responsive markers, semantic action-kind/destructive markers, conditional Delete separation, floating-template, lazy-state, disabled-tooltip, HTMX, modal, and refresh hooks. An override that changes only these controls does not own column placement.
