...
Current independent packs must declare template-pack contract version `5`. Version 5 adds the portable `extra_buttons_dropdown_label` rendering requirement to the version-4 list-toolbar contract; packs must render the supplied label for the list-level extra-button dropdown trigger, whose default is **Actions**. Version 4 added portable list-toolbar width and alignment settings to the version-3 row-action contract. Packs must preserve the toolbar's semantic width-policy and alignment markers, use the list container for `container`, keep `adjacent` controls together with a small gap, implement logical `split` spacing, and allow natural wrapping at narrow widths. Version 5 retains the version-3 start/end and sticky row-action columns, `extra_actions_mode = "all_dropdown"`, and responsive all-actions requirements. A declaration pinned to an older contract is rejected so consumers do not silently receive a pack that ignores these behaviours. Upgrade or regenerate the pack before selecting it with this PowerCRUD release.
