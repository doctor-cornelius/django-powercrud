...
Current independent packs must declare template-pack contract version `4`. Version 4 carries forward the portable start/end and sticky row-action column obligations from version 3 and adds `extra_actions_mode = "all_dropdown"`. The selected pack must render the resolved standard-action icon group and labelled extra-action rows, preserve configured colours and semantic tooltips, provide an accessible compact **Actions** trigger and screen-reader-only compact heading, and retain the detached-menu, lazy-state, HTMX, modal, guard, and disabled-state hooks. A declaration pinned to an older contract is rejected so consumers do not silently receive a pack that ignores the mode. Upgrade or regenerate the pack before selecting it with this PowerCRUD release.
