...
Current independent packs must declare template-pack contract version `3`. Version 3 adds portable start/end and sticky row-action columns, `extra_actions_mode = "all_dropdown"`, and the responsive all-actions collection. The selected pack must render desktop `dropdown_actions` in its supplied mode-specific scope and `responsive_dropdown_actions` as the canonical all-actions order below its narrow breakpoint. It must use labelled standard and configured menu rows, reserve a shared icon gutter only when the supplied dropdown collection has at least one `icon_html` action, keep icon-only controls accessible, provide accessible compact kebab triggers and a screen-reader-preserved compact heading, and retain the detached-menu, lazy-state, HTMX, modal, guard, and disabled-state hooks. Pack-native styling should keep ordinary controls and menu rows neutral, reserve destructive treatment for Delete, and avoid applying configured extra-button fills inside menus. A declaration pinned to an older contract is rejected so consumers do not silently receive a pack that ignores these behaviours. Upgrade or regenerate the pack before selecting it with this PowerCRUD release.
