...
| Row-action menus | Buttons and extras-only dropdown modes remain compatible on wider screens; both dropdown scopes use compact accessible kebabs; icon-only controls retain accessible names and semantic tooltips; all-dropdown follows the canonical labelled View, Edit, configured extras, Delete order. At the pack's narrow breakpoint, all modes expose only the canonical responsive all-actions menu. Menu items are neutral except for restrained destructive treatment; configured extra-button fills stay out of menus; Delete has no orphaned divider; inline Save/Cancel remain visible. Lazy state, disabled reasons, guards, modal/HTMX behavior, and replacement rows remain intact. |
...
- A version-3 pack honours row-action placement and horizontal stickiness across header, display, inline, and HTMX replacement rows, and renders both dropdown scopes with the canonical action semantics.
