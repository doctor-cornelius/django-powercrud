...
| Row-action menus | Buttons and extras-only dropdown modes remain compatible; all-dropdown renders only permitted standard actions in an accessible coloured icon group, labelled configured extras below it, semantic icon tooltips, no empty standard group, and visible inline Save/Cancel controls. Lazy state, disabled reasons, guards, modal/HTMX behavior, and replacement rows remain intact. |

Server tests should prove semantics and adapter decisions. Use browser tests only for behaviour that requires layout, focus, events, vendor code, or an actual HTMX/modal lifecycle. For row actions, assert resolved collections, permission filtering, accessible names, and semantic markers on the server, then use a small browser test to exercise horizontal scrolling, both floating-menu scopes, native and configured action activation, semantic tooltips, and inline Save/Cancel. Do not encode pixel tolerances, whitespace expectations, snapshots, or framework-class inventories as the portable contract.
