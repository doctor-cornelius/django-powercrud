...
- **CI matrix** covering the supported Python/Django versions (e.g., Python 3.12 w/ Django 4.2 & 5.0 initially).
...
- [ ] Build pytest configuration & fixtures (`conftest.py`, Django settings tweaks).
- [ ] Cover CoreMixin + table/pagination/url mixins with integration tests.
- [ ] Add FormMixin & FilteringMixin tests (including m2m AND/OR scenarios).
- [ ] Snapshot template tag output for list/detail rendering.
- [ ] Extend async tests to cover conflict tracking and cleanup.
- [ ] Draft Playwright smoke scenarios (modal CRUD + bulk operations).
- [ ] Wire coverage reporting into CI.

## Progress snapshot

_Nothing committed yet—this entry captures the initial plan. Update the checklist and add notes as each phase lands._
